...
function renderElementList(html: HtmlTemplateTag, state: SysideWorkspaceUiState, controller: SysideUiController, context: WorkspacePanelContext) {
  ...
  const selectedKey = state.selectedQualifiedName === undefined ? undefined : qualifiedNameKey(state.selectedQualifiedName);
  return state.elements.map((element) => html`
    <button
      type="button"
      class=${selectedKey === qualifiedNameKey(element.qualified_name) ? "syside-element-row is-selected" : "syside-element-row"}
      @click=${() => { controller.selectElement(context, element.qualified_name); }}
    >
      <span class="syside-element-short">${elementShortName(element)}</span>
...
const sysidePanelStyles = `
  ...
  .syside-panel .syside-element-row { display: grid; grid-template-columns: minmax(0, 1fr) minmax(0, 2fr) auto minmax(0, 2fr); gap: 8px; align-items: baseline; width: 100%; justify-content: flex-start; text-align: left; border: 0; border-radius: 0; background: transparent; margin: 0; padding: 5px 8px; }
  .syside-panel .syside-element-row:hover, .syside-panel .syside-element-row.is-selected { background: var(--pi-selection-bg); }
  .syside-panel .syside-element-short { font-weight: 600; }
  .syside-panel .syside-element-name, .syside-panel .syside-element-type, .syside-panel .syside-element-qn { color: var(--pi-muted); }
  .syside-panel .syside-element-type { text-transform: uppercase; font-size: 11px; letter-spacing: .03em; }
